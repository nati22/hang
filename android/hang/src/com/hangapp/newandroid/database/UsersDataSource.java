...
public final class UsersDataSource {
	...
	private String[] allColumns = { MySQLiteHelper.COLUMN_JID,
			...
			MySQLiteHelper.COLUMN_INCOMING_BROADCASTS,
			MySQLiteHelper.COLUMN_OUTGOING_BROADCASTS };
	...
	public boolean saveUserInDatabase(User user) {
		...
		String incomingStringArray = Utils.convertStringArrayToString(user
				.getIncomingBroadcasts());
		String outgoingStringArray = Utils.convertStringArrayToString(user
				.getOutgoingBroadcasts());
		...
		// If the user has an Incoming Broadcasts string array, then save it in
		// the SQLite row.
		if (incomingStringArray != null) {
			values.put(MySQLiteHelper.COLUMN_INCOMING_BROADCASTS,
					incomingStringArray);
		}

		// If the user has an Outgoing Broadcasts string array, then save it in
		// the SQLite row.
		if (outgoingStringArray != null) {
			values.put(MySQLiteHelper.COLUMN_OUTGOING_BROADCASTS,
					outgoingStringArray);
		}
